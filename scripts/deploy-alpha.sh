...
#   NAMESPACE             Target namespace (default: gough-alpha)
#   APP_HOST              Application hostname (default: gough.localhost.local)
#
# =============================================================================
...
readonly APP_NAME="${APP_NAME:-gough}"
readonly KUBE_CONTEXT="${KUBE_CONTEXT:-local-alpha}"
readonly NAMESPACE="${NAMESPACE:-gough-alpha}"
readonly APP_HOST="${APP_HOST:-gough.localhost.local}"
readonly OVERLAY_PATH="${OVERLAY_PATH:-k8s/kustomize/overlays/alpha}"

# Services with Dockerfiles (customize per repo)
declare -a SERVICES=("access-agent" "api-manager" "webui" "worker-ipxe")
...
EXAMPLES:
    ...
    $(basename "$0") --service access-agent
